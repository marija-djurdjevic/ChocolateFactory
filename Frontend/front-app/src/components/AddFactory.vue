    ...
    <form @submit.prevent="submitForm">
      ...
      <div class="form-group">
        ...
        <select id="manager" v-model="selectedManager" required>
          <option v-for="manager in managers" :key="manager" :value="manager">{{ manager.name }}  {{ manager.surname }}</option>
        </select>
      </div>
...
function submitForm() {
  if (selectedFile.value) {
    ...
    reader.onload = (e) => {
      factory.value.logo = e.target.result.split(",")[1];
      saveFactory();
    }
...
function saveLocation() {
  axios.post('http://localhost:8080/WebShopAppREST/rest/locations/save', location.value)
    .then(response => {
