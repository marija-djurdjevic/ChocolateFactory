<template>
  <div class="users-display">
    ...
    <div class="filter-bar">
      ...
      <!-- Button to fetch suspicious customers -->
      <button class="susButton" @click="fetchSuspiciousCustomers">Suspicious</button>
    </div>
...
// Function to fetch suspicious customers
const fetchSuspiciousCustomers = async () => {
  try {
    const response = await axios.get('http://localhost:8080/WebShopAppREST/rest/purchases/suspiciousCustomers');
    const suspiciousCustomers = response.data;
    console.log('Suspicious Customers:', suspiciousCustomers);
    // Handle how you want to display or use suspicious customers
    // For example, you can update displayedUsers to show suspicious customers only
    displayedUsers.value = users.value.filter(user => {
      return suspiciousCustomers.some(suspicious => suspicious.username === user.username);
    });
  } catch (error) {
